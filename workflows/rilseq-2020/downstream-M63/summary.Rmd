...
knitr::opts_chunk$set(
    ...
    cache.extra_file_dep_2 = file.info('../data/rnaseq_rilseq/*/*.svg')$mtime
    )
```
...
```{r data}
subgroup <- '_M63'
cat(paste0('\n\nHere we subset the samples to ', subgroup, ' samples.\n\n'))
spt <- read.csv('../config/sampletable.tsv', sep='\t')
# subset to subgroup
spt <- spt %>%
  filter(grepl(subgroup, spt[['samplename']]))
knitr::kable(spt %>%
                select(-layout)
                )
```
...
```{r, results='asis'}
cat(paste0('\n\n**Link to output file:** [Summary_RILSeq_S-chimeras_2020_unified_', gsub('\\|', '_', subgroup),
  '.xlsx](Summary_RILSeq_S-chimeras_2020_unified_', gsub('\\|', '_', subgroup), '.xlsx).'))
```
...
generate the `Summary_RILSeq_S-chimeras.xlsx` file.

The updated annotation [ecoli_default.patch1.gff](../../annotation/data/ecoli_default.patch1.gff)
...
unless the percentage of RNA reads overlapping with a gene is more than the min_ratio (0.7), in that case
...
```{r get_RNAnames, results='asis', cache=TRUE, dependson='read_chimeras_input', eval=FALSE}
# matching RNA names between significant fragments and all chimera fragemnts

# to make GRanges, need those columns:
...
for (sample.i in spt[['samplename']]){
#  print(sample.i)
  if (!is.null(nrow(sig.chimeras[[sample.i]])) & (nrow(sig.chimeras[[sample.i]]) > 1)) {
    RNA_names[[sample.i]] <- cbind(RNA1_name=rep("", nrow(sig.chimeras[[sample.i]])),
                                 RNA2_name=rep("", nrow(sig.chimeras[[sample.i]])))

    # for each chimera, get all associated reads 
    for(chi.i in 1:nrow(sig.chimeras[[sample.i]])) {
      target_chimera <- sig.chimeras[[sample.i]][chi.i,]
      target_fragments <- getChimericFragmentsByGRanges(fragment.df = fragments[[sample.i]] %>%
                                                                      filter(type == 'chimera'),
      ...
      RNA_names[[sample.i]][chi.i,] <- c(RNA1_names, RNA2_names)
    }
  }
}
```
...
exported_excel(sig.chimeras, unified_fns, file=paste0('Summary_RILSeq_S-chimeras_2020_',
  gsub('\\|', '_', subgroup), '.xlsx'))
```
...
exported_excel_unified(sig.unified, unified_fns,
  file=paste0('Summary_RILSeq_S-chimeras_2020_unified_', gsub('\\|', '_', subgroup), '.xlsx'))
```
